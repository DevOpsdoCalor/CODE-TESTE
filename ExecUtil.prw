...
Auxiliar na execu��o da de rotinas sem necessidade de acesso ao 
sistema.
...
Method Execute() Class ExecUtil
    ...
    aAdd(aButtons,{"PESQUISA",OemToAnsi( "&Limpar Hist�rico" ),bAction1 ,OemToAnsi( "Limpa hist�rico de fun��es..." ),,.T.,.F.})
    ...
    oTComboBox := TComboBox():New(nCol,nLin,{|u| If(PCount()>0, Self:jExecUtil['CURRENT'] := u, Self:jExecUtil['CURRENT'])},aHistory/*ITEMS*/,180,15,oContainer,,{||Self:ValidFunction(.T.)},{|| Self:ValidFunction(.T.) },,,.T.,,,,,,,,,"Self:jExecUtil['CURRENT']","Fun��o",1) 
...
Method ValidLogin() Class ExecUtil
    ...
    If  PswSeek(AllTrim(Self:jExecUtil['USERNAME'])) 
        If PswName(AllTrim(Self:jExecUtil['PASSWORD'])) 
            lRet := .T.     
        Else
            cMsg := "<b>Problema: </b>Senha do usu�rio <b>" + Alltrim(Self:jExecUtil['USERNAME']) + "</b> inv�lida." + CRLF + CRLF
            cMsg += "<b>Solu��o: </b>Digite a senha novamente."
            jModal['PASSWORD']:SetFocus() 
        EndIf      
    Else 
        cMsg := "<b>Problema: </b>Usu�rio <b>" + Alltrim(Self:jExecUtil['USERNAME']) + "</b> n�o encontrado." + CRLF + CRLF
        cMsg += "<b>Solu��o: </b>Informe um nome de usu�rio cadastrado."
...
Method ValidSM0(cType) Class ExecUtil
    ...
    If aScan(Self:aSM0,{|x|x[1]==Self:jExecUtil["EMPRESA"]}) == 0
        ...
        cMsg := "<b>Problema: </b>C�digo <b>" + Alltrim(Self:jExecUtil['EMPRESA']) + "</b> de empresa inv�lido." + CRLF + CRLF + CRLF
        cMsg += "<b>Solu��o: </b>Digite novamente a senha."
    ...
    ElseIf aScan(Self:aSM0,{|x|x[1]==Self:jExecUtil["EMPRESA"] .And. x[2]==Self:jExecUtil["FILIAL"]}) == 0
        ...
        cMsg := "<b>Problema: </b>C�digo <b>" + Alltrim(Self:jExecUtil['FILIAL']) + "</b> de filial inv�lido." + CRLF + CRLF + CRLF
        cMsg += "<b>Solu��o: </b>Informe um c�digo v�lido."
...
Method Login() Class ExecUtil
    ...
    TSay():New(nLin+nSay,nColSay,{||"Usu�rio"},oContainer,,,,,,.T.,,,69,12,,,,,,)
